...
class SurfaceStressDataWriter(object):
    ...
    def compute_monthly_mean_fields(self, date_in_month, method):
        ...
        # FIXME: tmp fix to get correct monthly means
        # This hack will overwrite the calculated fields (tau_air, tau_ice, tau, tau_SIZ, u_Ekman, etc.) using the
        # monthly averaged data from above AND the monthly mean sea ice motion dataset.
        # u_ice_field_avg = np.zeros((len(self.lats), len(self.lons)))
        # v_ice_field_avg = np.zeros((len(self.lats), len(self.lons)))
        #
        # logger.info('Calculating monthly surface stress field (tau_x, tau_y)...')
        #
        # sea_ice_motion_data = SeaIceMotionDataReader(date_in_month, monthly=True)
        #
        # for i in range(len(self.lats)):
        #     lat = self.lats[i]
        #     f = 2 * Omega * np.sin(np.deg2rad(lat))  # Coriolis parameter [s^-1]
        ...
        #         lon = self.lons[j]
        #
        #         # Using the computed monthly averages for these.
        ...
        #         alpha = alpha_field[i][j]
        #
        #         # Grabbing this value from the monthly mean dataset.
        #         u_ice_vec = sea_ice_motion_data.seaice_motion_vector(lat, lon, date_in_month, 'interp')
        #
        #         # If there's no sea ice at a point and we have data at that point (i.e. the point is still in the ocean)
        ...
        #             continue
        #
        #         # Otherwise we're in the sea ice zone and can calculate tau_air and tau_ice.
        #         tau_vec, tau_air_vec, tau_ice_vec = self.surface_stress(f, u_geo_vec, u_wind_vec, alpha, u_ice_vec)
        #
        #         self.tau_air_x_field[i][j] = tau_air_vec[0]
        #         self.tau_air_y_field[i][j] = tau_air_vec[1]
        ...
        #         self.v_Ekman_SIZ_field[i][j] = u_Ekman_vec[1]
        #
        # self.compute_daily_ekman_pumping_field()  # FIXME: Not actually a daily field but this will work.
